...
class FunctionPlottingService:
    ...
    def _create_function(self, function_str: str) -> Tuple[Callable, sp.Expr]:
        """Create a callable function from a string expression"""
        x = sp.symbols('x')
        expr = sp.sympify(function_str)
        return sp.lambdify(x, expr, "numpy"), expr

    def _find_critical_points(self, function_str: str, x_min: float, x_max: float, 
                             func: Optional[Callable] = None, expr: Optional[sp.Expr] = None) -> List[float]:
        x = sp.symbols('x')
        try:
            if expr is None:
                _, expr = self._create_function(function_str)
    ...
    def _find_inflection_points(self, function_str: str, x_min: float, x_max: float,
                               func: Optional[Callable] = None, expr: Optional[sp.Expr] = None) -> List[float]:
        x = sp.symbols('x')
        try:
            if expr is None:
    ...
    def _detect_asymptotes(self, function_str: str, x_min: float, x_max: float, 
                         func: Optional[Callable] = None) -> List[float]:
        try:
            if func is None:
                func, _ = self._create_function(function_str)
    ...
    def adaptive_sampling(self, function_str: str, x_min: float, x_max: float, 
                      precision: int = 100, max_points: int = 3000,
                      func: Optional[Callable] = None, expr: Optional[sp.Expr] = None) -> Tuple[np.ndarray, np.ndarray, list, list]:
        try:
            if func is None or expr is None:
                func, expr = self._create_function(function_str)
            
            x_initial = np.linspace(x_min, x_max, precision)
            
            critical_points = self._find_critical_points(function_str, x_min, x_max, func, expr)
            inflection_points = self._find_inflection_points(function_str, x_min, x_max, func, expr)
            asymptotes = self._detect_asymptotes(function_str, x_min, x_max, func)
            ...
            for point in critical_points:
                if x_min <= point <= x_max:
                    critical_data_points.append([point, float(func(point))])
                    
            for point in inflection_points:
                if x_min <= point <= x_max:
                    inflection_data_points.append([point, float(func(point))])
            ...
            for i in range(len(x_values) - 1):
                ...
                if abs(y_values[i+1] - y_values[i]) > 10 * np.nanstd(y_values):
                    ...
                    intermediate_y = func(intermediate_x)
    ...
    def generate_plot_data(self, function: str, lower_bound: float, upper_bound: float, 
                         precision: int, max_points: int) -> dict:
        try:
            ...
            # Convert from standard math notation to Python expression
            function = self._convert_to_python_expr(function)
            
            # Create the function once and pass it to all methods
            func, expr = self._create_function(function)
